...
const BusinessMap = dynamic(() => import('@/components/BusinessMap'), { ssr: false, loading: () => (
  <div style={{height:450,background:'#0D0E12',borderRadius:12,border:'1px solid rgba(255,255,255,0.08)',display:'flex',alignItems:'center',justifyContent:'center'}}>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Map businesses — all with coords
      const { data: mapData } = await supabase.from('businesses')
        ...
        .eq('status','active').not('latitude','is',null).limit(200)
      if (mapData?.length) {
      ...
      setActivity([
        {icon:'🏢',label:'158 businesses live',sub:'Delhi NCR + 8 major cities',color:'#C9A84C'},
        {icon:'📍',label:'Phase 3 SEO pages live',sub:'/pincode, /digipin, /post-office',color:'#22c55e'},
        {icon:'📧',label:'Email system connected',sub:'Resend API — 3,000/month free',color:'#60a5fa'},
        {icon:'🔐',label:'Auth system secured',sub:'Magic link + password login',color:'#a78bfa'},
        {icon:'🗺️',label:'Interactive map added',sub:'Leaflet.js with real coordinates',color:'#fb923c'},
      ])
      setLoading(false)
    }
    init()
  ...
  const statGrid = [
    {label:'Total Listings',value:stats?.total,icon:'🏢',color:G,sub:'all businesses'},
    {label:'Active',value:stats?.active,icon:'✅',color:'#22c55e',sub:'live now'},
    {label:'Pending',value:stats?.pending,icon:'⏳',color:'#f59e0b',sub:'review queue'},
    {label:'Featured',value:stats?.featured,icon:'⭐',color:G,sub:'premium'},
    {label:'Verified',value:stats?.verified,icon:'🔵',color:'#60a5fa',sub:'verified'},
    {label:'Reviews',value:stats?.reviews,icon:'💬',color:'#a78bfa',sub:'total'},
    {label:'Cities',value:stats?.cities,icon:'🌆',color:'#34d399',sub:'covered'},
    {label:'Page Views',value:(stats?.views||0).toLocaleString(),icon:'👁️',color:'#94a3b8',sub:'total'},
  ]
  ...
  return (
    <div style={{minHeight:'100vh',background:'#070809',fontFamily:"'DM Sans',sans-serif",paddingTop:68}}>
      <style>{`
        ...
        .leaflet-popup-content-wrapper{border-radius:10px!important;box-shadow:0 4px 20px rgba(0,0,0,0.4)!important}
      `}</style>
        ...
        <main style={{flex:1,overflowY:'auto',padding:'28px 24px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
            <div>
              ...
              <p style={{color:'#475569',fontSize:13,margin:'2px 0 0'}}>SKM Studio Maps — {stats?.total} businesses live</p>
            </div>
            ...
            <div className="fadein">
              ...
              <div style={{marginBottom:20}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                  ...
                  <button onClick={()=>setSideTab('map')} style={{background:'rgba(201,168,76,0.1)',border:'1px solid rgba(201,168,76,0.3)',color:G,padding:'6px 14px',borderRadius:8,cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:600}}>Open Full Map →</button>
                </div>
                <BusinessMap businesses={mapBiz} height="320px" />
              </div>
              {/* Recent table */}
            ...
            <div className="fadein">
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:10}}>
                <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                  <span style={{color:'#8a8da0',fontSize:13}}>Filter by city:</span>
                  <button onClick={()=>setMapCity('all')} style={{background:mapCity==='all'?'rgba(201,168,76,0.15)':'rgba(255,255,255,0.04)',border:`1px solid ${mapCity==='all'?'rgba(201,168,76,0.4)':'rgba(255,255,255,0.1)'}`,color:mapCity==='all'?G:'#8a8da0',padding:'5px 12px',borderRadius:99,cursor:'pointer',fontSize:12,fontFamily:"'DM Sans',sans-serif"}}>All ({mapBiz.length})</button>
                  {allCities.slice(0,10).map(c=>{
                    const count = mapBiz.filter(b=>b.city===c).length
                    return <button key={c} onClick={()=>setMapCity(c)} style={{background:mapCity===c?'rgba(201,168,76,0.15)':'rgba(255,255,255,0.04)',border:`1px solid ${mapCity===c?'rgba(201,168,76,0.4)':'rgba(255,255,255,0.1)'}`,color:mapCity===c?G:'#8a8da0',padding:'5px 12px',borderRadius:99,cursor:'pointer',fontSize:12,fontFamily:"'DM Sans',sans-serif"}}>{c} ({count})</button>
                  })}
                </div>
                <div style={{color:'#475569',fontSize:12}}>{filteredMap.length} pins shown</div>
              </div>
              <BusinessMap businesses={filteredMap} height="calc(100vh - 230px)" />
            </div>
          )}

          {/* SEO TAB */}
          {sideTab==='seo' && (
            <div className="fadein" style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:16}}>
              {[
                ...
                {title:'Add Your Business',sub:'Free listing available',href:'/add-business',icon:'➕',color:'#fb923c'},
              ].map(p=>(
                <Link key={p.title} href={p.href} style={{background:'#0D0E12',border:`1px solid ${p.color}20`,borderRadius:12,padding:'20px',textDecoration:'none',display:'block',transition:'all 0.2s'}}
                  ...
                  <div style={{color:p.color,fontSize:12,fontWeight:600,marginTop:12}}>Open →</div>
                </Link>
              ))}
            </div>
          )}
